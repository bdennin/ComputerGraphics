...
public class SimpleRayTracing {
	...
	private static final BufferedImage FRAME = new BufferedImage(800, 800, BufferedImage.TYPE_INT_RGB);
	...
	private static final int WIDTH = FRAME.getWidth() - 1;
	private static final int HEIGHT = FRAME.getHeight() - 1;
	private static final int TOTAL_SURFACES = SURFACES.size(); 
	private static final int SQUARE_LENGTH = 4;
	private static final int LIGHT_ITERATIONS = 3000;
	...
	private static void drawPicture(int x, int xMax, int y, int yMax) {
		if(xMax - x <= SQUARE_LENGTH || yMax - y <= SQUARE_LENGTH) {
			
			for(int i = y; i <= yMax; i++) {
				for(int j = x; j <= xMax; j++) {
					
					double a = (-1 + 2 * (double)i/WIDTH);
					double b = (1 - 2 * (double)j/HEIGHT);
					double c = -1;
			...
			int xAverage = (xMax + x)/2;
			int yAverage = (yMax + y)/2;
			
			drawPicture(x, xAverage, y, yAverage);
			drawPicture(xAverage + 1, xMax, y, yAverage);
			drawPicture(x, xAverage, yAverage + 1, yMax);
			drawPicture(xAverage + 1, xMax, yAverage + 1, yMax);
		}
	}
...
//		surfaces.add(new Disc(new Vector3D(-122, 0, 0), new Vector3D(-123, -3, 0), new Vector3D(-119, -2, 10), 6, 10, "uranusRings.png"));
//		surfaces.add(new Sphere(new Vector3D(-260, 0, 0), 4, "neptune.jpg"));
		surfaces.add(new Orb(new Vector3D(0, 10, -10), 1));
		surfaces.add(new Sphere(new Vector3D(0, 1, -10), 1, Color.RED));
